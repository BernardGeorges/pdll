...
class PairwiseDifferenceBase(sklearn.base.BaseEstimator):
    ...
    def pair_input_training(X1, X2):  # -> tuple[pd.DataFrame, pd.DataFrame]:
        ...
        x2_pair_sym = X_pair[[f'{col}_x' for col in X1.columns if col != '__idx1']].rename(columns={f'{col}_x': f'{col}_y' for col in X1.columns if col != '__idx1'})
        x1_pair_sym = X_pair[[f'{col}_y' for col in X1.columns if col != '__idx1']].rename(columns={f'{col}_y': f'{col}_x' for col in X1.columns if col != '__idx1'})
